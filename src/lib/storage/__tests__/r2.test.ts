...
describe("private Cloudflare R2 foundation", () => {
  const previousEnv = {
  ...
  beforeEach(() => {
    ...
    process.env.R2_BUCKET_NAME = "aicare-clinical-uploads";
  });
  ...
  it("loads R2 configuration from the environment", () => {
    expect(loadR2Config()).toEqual({
      accountId: "test-account",
      accessKeyId: "test-access-key",
      secretAccessKey: "test-secret-key",
      bucketName: "aicare-clinical-uploads",
    });
  });
