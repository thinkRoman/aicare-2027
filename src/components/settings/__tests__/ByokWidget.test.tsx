...
describe("ByokWidget", () => {
  ...
  it("marks reserved providers unavailable and allows local LM Studio config", async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes("/api/byok") && (!init || init.method === undefined || init.method === "GET")) {
    ...
    expect(
      screen.getByText(/encrypted before storage and is never shown again/i),
    ).toBeInTheDocument();
    ...
    const putCall = fetchMock.mock.calls.find(
      (call) =>
        String(call[0]).includes("/api/byok") &&
        (call[1] as RequestInit | undefined)?.method === "PUT",
    );
    expect(putCall?.[1]?.body).toContain("http://127.0.0.1:1234/v1");
    expect(putCall?.[1]?.body).toContain("qwen2.5-7b-instruct");

    expect(await screen.findByText(/key ending/i)).toBeInTheDocument();
    expect(screen.getByText(/••••ocal/i)).toBeInTheDocument();
    expect(screen.queryByDisplayValue("lm-studio-local-key")).not.toBeInTheDocument();
  ...
  it("supports explicit disable and delete actions", async () => {
    ...
    render(<ByokWidget authenticated />);
    expect(await screen.findByText(/status:/i)).toHaveTextContent(/enabled/i);

    await user.click(screen.getByRole("button", { name: /^disable$/i }));
    await waitFor(() => {
      expect(screen.getByText(/status:/i)).toHaveTextContent(/disabled/i);
    });
